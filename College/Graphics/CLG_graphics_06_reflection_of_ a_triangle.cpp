#include<iostream.h>
#include<math.h>
#include<dos.h>
#include<math.h>
#include<conio.h>
#include<graphics.h>
#include<stdlib.h>
void main()
{
int gd = DETECT, gm, i, xmid, ymid, x1, y1, x2, y2, x3, y3, x, y, dx, dy;
int p, gap = 50, temp, option, x1dash, x2dash, x3dash;
int y1dash, y2dash, y3dash;
double theta;
float m;
char str[];
clrscr();
initgraph(&gd, &gm, "C:\\TurboC3\\BGI");
cout<<"Enter first coordinates of Triangle";
cin>>x1>>y1;
cout<<"Enter Second coordinates of Triangle";
cin>>x2>>y2;
cout<<"Enter Third coordinates of Triangle";
cin>>x3>>y3;
xmid = getmaxx()/2;
ymid = getmaxy()/2;
line(5,ymid, getmaxx() - 5,ymid);
line(xmid + 3.5, xmid + 3, getmaxy() - 5);
for(i = xmid + gap; i < getmaxx() - 5; i = i+gap)
{
outtextxy(i, ymid - 3, "1");
itoa(i-xmid, str, 10);
outtextxy(i, ymid + 3, str);
}
for(i = ymid - gap; i>5; i = i -gap)
{
outtextxy(xmid, i, "-");
itoa(ymid - i, str, 10);
outtextxy(xmid + 5, i, str);
}
line(x1 + xmid, ymid - y1, x2 + xmid, ymid - y2);
line(x2 + xmid, ymid - y2, x3 + xmid, ymid - y3);
line(x3+xmid, ymid - y3, x1+xmid, ymid - y1);
setcolor(255);
cout<<"Reflection about";
cout<<"x axis: 1";
cout<<"y axix: 2";
cout<<"x = y axis: 3";
cin>>option;
switch(option)
{
case 1: y1 = -y1;
y2 = -y2;
y3 = -y3;
break;
case 2: x1 = -x1;
x2 = -x2;
x3 = -x3;
break;
case 3: y1 = -y1;
MRIDUL GUPTA 01916401518 13
y2 = -y2;
y3 = -y3;
theta = ((double)90*3.14)/(double)180;
x1dash = x1*cos(theta) - y1*sin(theta);
x2dash = x2*cos(theta) - y2*sin(theta);
x3dash = x3*cos(theta) - y3*sin(theta);
y1dash = x1*sin(theta) + y1*cos(theta);
y2dash = x2*sin(theta) + y2*cos(theta);
y3dash = x3*sin(theta) + y3*cos(theta);
x1 = x1dash;
x2 = x2dash;
x3 = x3dash;
y1 = y1dash;
y2 = y2dash;
y3 = y3dash;
}
line(x1+xmid, ymid - y1, x2 + xmid, ymid - y2);
line(x2 + xmid, ymid - y2, x3 + xmid, ymid - y3);
line(x3 + xmid, ymid - y3, x1 + xmid, ymid - y1);
getch();
closegraph();
}